...
class SendData2(object):
    ...
    def post_data(self, command, data, return_cmd_data=False):
        ...
        if answer.status_code == 204:
            raise Exception('{}'.format(
                "Помилка 204. На фіскальному сервері немає об'єктів для роботи, будь ласка перевірити форму 5-ПРРО в "
                "ЛК ДПС, якщо недавно були відправлені форми реєстрації, чекайте, потрібен час для синхронізації між "
                "серверами податкового кабінету"))
